...
function App() {
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      {!editor && <div className="file-start"><div className="file-start-heading"><h2>Open your Seaglass files</h2><p>Drop both files below or browse for them. Files stay on this device, and the ROM is remembered locally after the first selection.</p></div><div className="file-drop-grid"><FileDropZone kind="ROM" detail="Choose your Pokémon Emerald Seaglass .gba file." ready={Boolean(rom)} onBrowse={() => romInput.current?.click()} onFile={chooseRom} onReject={() => setNotice("That is not a .gba ROM file.")} /><FileDropZone kind="Save" detail="Choose the .sav or .srm file you want to edit." ready={Boolean(saveBuffer)} onBrowse={() => saveInput.current?.click()} onFile={chooseSave} onReject={() => setNotice("That is not a .sav or .srm save file.")} /></div></div>}
